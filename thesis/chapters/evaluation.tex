...
Haindl \cite{haindl} claims to have the MAE equal to $3.16$ in CIELAB color space for the same sample.
 ...
 In this case we would need approximately $41$ components \cite{haindl}, which is the heavy computational task for the GPU.
 
 Also, the LPCA BTF \cite{haindl} method has the MAE equal to $2.42$, which is practically equal to our method. 
 However, we use $8$ components, while the LPCA BTF method uses $19$ on average.
