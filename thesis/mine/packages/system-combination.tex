...
\begin{tikzpicture}
   ...
   \path (vote.south)+(0.0,-2.0) node (syscomb) [sc] {Shader};
   \path (syscomb.west)+(-2,0) node (decompress) [sensor] {Decompression};
   \path (decompress.west)+(-2,0) node (interp) [sensor] {Light and Camera View Interpolations};
    \path (interp.west)+(-2,0) node (color) [final] {Final Color};
	
     \path [draw, ->] (vote.south) -- node [above] {} 
        (syscomb.90);
		
   \path [draw, ->] (syscomb.west) -- node [left] {} 
        (decompress.0);
		 \path [draw, ->] (decompress.west) -- node [left] {} 
        (interp.0);
			 \path [draw, ->] (interp.west) -- node [left] {} 
        (color.0);
